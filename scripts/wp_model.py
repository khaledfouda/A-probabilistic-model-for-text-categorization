...
class Proto:
    ...
    def predict(self, x=pd.Series()):
        ...
        """
        intersections = x.swifter.allow_dask_on_strings().apply(lambda d: self.p_y_wp.index.intersection(d.split()))
        pred = intersections.swifter.apply(lambda d: self.p_y_wp.loc[d, :].sum(axis=0).argmax())
        flag = intersections.swifter.apply(lambda d: len(d) == 0)
        return pred, flag
    # --------------------------------------------------------------------------------
